...
class Job {
  ...
  static async apply(id, username, state) {
    ...
    /** Verify if application is already in db */
    const verify = await db.query(
      `SELECT job_id, username, state
      FROM applications
    ...
    if(verify.rows.length === 0) {
      ...
      // application exists, toggle state
      const newState = verify.rows[0].state === "applied" ? "closed" : "applied";
      await db.query(
        `UPDATE applications 
        SET state =  $1 
